...
>(
  ...
  async (branchId, { rejectWithValue }) => {
    try {
      ...
      return response.data.data; // data: Bin[] /findName
    } catch (error: any) {
      return rejectWithValue(
        error.response?.data?.message || 'Échec de la récupération des bins'
      );
  ...
  export const findByBinName = createAsyncThunk< 
  Bin ,   
  string, 
  AsyncThunkConfig
>(
  ...
  async (name, { rejectWithValue }) => {
    try {
      const response = await API.get(`bin/findName/${name}`);
 ...
 export const findByBranchType = createAsyncThunk< 
  Bin [],   
  BinBranchType, 
  AsyncThunkConfig
>(
  'bin/getBinBranchType',
  async (data, { rejectWithValue }) => {
    try {
      const response = await API.get(`bin/find/${data.id}/${data.type}`);
