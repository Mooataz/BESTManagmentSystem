...
const DynamicTable: React.FC<TableProps> = ({ rows ,clickedRowId , columnsToShow, columnLabels, actions }) => {
  const { t } = useTranslation();
    ...
    <> 
      ...
      <Table sx={{ minWidth: 650 }} >
        <TableHead>
          <TableRow>
            {visibleColumns.map((col) => (
              <TableCell key={col} style={{ border: '1px solid #ccc', padding: '8px', color: theme.palette.primary.main , fontWeight: "bold", }}>
                  {columnLabels?.[col] || col}
              </TableCell>
            ))}
            <th style={{ border: '1px solid #ccc', padding: '8px', color: theme.palette.primary.main , fontWeight: "bold", }}>Actions</th>
          </TableRow>
        </TableHead>
...
<TableBody>
  ...
  {Array.isArray(value) ? (
    ...
    value === 'Autoriser' ? (
      <Card  sx={{ borderColor:'green', borderRadius:'50px'}}   >
        {value}
      </Card>
    ) : (
      <Card  sx={{ borderColor:'red', borderRadius:'50px'}}>
        {value}
      </Card>
    )
  ) : col.toLowerCase().includes('date') && !isNaN(Date.parse(value)) ? (
  ...
  value
     ...
     {actions && (
        <TableCell style={{ border: '1px solid #ccc', padding: '8px' }}>
          {actions.map((action, index) => (
            <span key={index} onClick={() => action.onClick(row)} style={{ marginLeft: '20px', cursor: 'pointer' }}>
              {action.icon}  
            </span>
          ))}
        </TableCell>
