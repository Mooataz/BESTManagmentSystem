...
import { getAllExpertiseRaisons, UpdateOneRaison } from '../../../Redux/Actions/Administration/RaisonsExpertiseActions';
import { useNotification } from '../../../Componants/NotificationContext';
interface UpdateRaisonProps {
  raison: any;
  open: boolean;
  onClose: () => void;
}
...
export function UpdateExpertiseRaison({ raison, open, onClose }: UpdateRaisonProps) {
  ...
  React.useEffect(() => {
    if (raison) {
      ...
      setDescription({ id: raison.id, name: raison.name })
  ...
  setIsLoading(true);
 try {
  await dispatch(UpdateOneRaison(description)) 
      .then( ( ) => {
  ...
  return (
    <Box>
      ...
      >
        <DialogTitle>{"Modifier un raison"}</DialogTitle>
        <DialogContent>
          <form
              ...
              <FormControl>
                ...
                <Box sx={{ display: 'flex' }}>
                  ...
                  <Button size="medium" variant={'outlined'} color="info" style={{ width: '160px', marginLeft: '14%' }} loading={isLoading}
                    onClick={() => handleSubmit({ id: raison.id, name: description.name })} > Mettre à jour</Button>
