...
import { getListFault, UpdateListFault } from '../../../Redux/Actions/Administration/ListFaultActions';
...
interface UpdateProblemsProps {
    problem: any;
    open: boolean;
    onClose: () => void;
}
export default function UpdateProblem({ problem, open, onClose }: UpdateProblemsProps) {
    ...
    React.useEffect(() => {
        if (problem) {
            ...
            setDescription({ id: problem.id, name: problem.name })
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            await dispatch(UpdateListFault(updateData)).unwrap();
            await dispatch(getListFault()); // Rafraîchir la liste
    ...
    return (
        <Dialog
            ...
            <DialogTitle>{"Modifier un problème"}</DialogTitle>
            <DialogContent>
                <Stack spacing={2}>
                    <FormControl>
