...
} from "./chunk-DCXGITF4.js";
import {
  useFormControlContext,
  useInput
} from "./chunk-VQRMLFNH.js";
import {
  generateUtilityClass,
  generateUtilityClasses
} from "./chunk-65J37A7M.js";
import {
  useButton
...
function useOption(params) {
  ...
  const optionRef = React4.useRef(null);
  const selectOption = React4.useMemo(() => ({
    disabled,
    label,
    value,
    ref: optionRef,
...
function useOptionContextStabilizer(value) {
  ...
  const localGetItemState = React5.useCallback((itemValue) => {
    if (itemValue !== value) {
  ...
  const localContextValue = React5.useMemo(() => ({
    dispatch,
...
function useSlider(parameters) {
  ...
  const touchId = React8.useRef(void 0);
  ...
  const previousIndex = React8.useRef(void 0);
...
function useTabs(parameters) {
  ...
  const onSelected = React10.useCallback((event, newValue) => {
    setValue(newValue);
  ...
  const tabIdLookup = React10.useRef(() => void 0);
  const getTabPanelId = React10.useCallback((tabValue) => {
    var _tabPanels$get;
    return (_tabPanels$get = tabPanels.get(tabValue)) == null ? void 0 : _tabPanels$get.id;
  }, [tabPanels]);
  const getTabId = React10.useCallback((tabPanelId) => {
    return tabIdLookup.current(tabPanelId);
  }, []);
  const registerTabIdLookup = React10.useCallback((lookupFunction) => {
...
function TabsProvider(props) {
  ...
  const tabsContextValue = React12.useMemo(() => ({
    direction,
    getTabId,
    getTabPanelId,
    onSelected,
...
var Tabs = React13.forwardRef(function Tabs2(props, forwardedRef) {
  var _slots$root;
  const {
    children,
    orientation = "horizontal",
...
function useTabPanel(parameters) {
  ...
  const metadata = React14.useMemo(() => ({
    id,
    ref
  }), [id]);
  const {
...
function useTabsList(parameters) {
  ...
  const tabIdLookup = React15.useCallback((tabValue) => {
    var _subitems$get;
  ...
  const getTabElement = React15.useCallback((tabValue) => {
  ...
  const handleChange = React15.useCallback((event, newValue) => {
    var _newValue$;
    onSelected(event, (_newValue$ = newValue[0]) != null ? _newValue$ : null);
  }, [onSelected]);
  const controlledProps = React15.useMemo(() => {
    if (value === void 0) {
      return {};
    }
    return value != null ? {
  ...
  const isItemDisabled = React15.useCallback((item) => {
  ...
  const contextValue = React15.useMemo(() => _extends({}, compoundComponentContextValue, listContextValue), [compoundComponentContextValue, listContextValue]);
  return {
    contextValue,
    dispatch,
    getRootProps,
...
function useTab(parameters) {
  ...
  const tabRef = React17.useRef(null);
  const id = useId(idParam);
  const {
    value: selectedValue,
    selectionFollowsFocus,
  ...
  const tabMetadata = React17.useMemo(() => ({
    disabled,
    ref: tabRef,
    id
  }), [disabled, tabRef, id]);
...
function useNumberInput(parameters) {
  ...
  const handleStateChange = React33.useCallback((event, field, fieldValue, reason) => {
  ...
  const numberInputActionContext = React33.useMemo(() => {
    return {
      min: min2,
      max: max2,
      step,
  ...
  const [state, dispatch] = useControllableReducer({
    ...
    initialState,
    onStateChange: handleStateChange,
    actionContext: React33.useMemo(() => numberInputActionContext, [numberInputActionContext]),
    componentName
  });
  const {
    value,
    ...
    children: [(0, import_jsx_runtime27.jsx)(Rail, _extends({}, railProps)), (0, import_jsx_runtime27.jsx)(Track, _extends({}, trackProps)), marks.filter((mark) => mark.value >= min2 && mark.value <= max2).map((mark, index2) => {
      ...
      return (0, import_jsx_runtime28.jsxs)(Thumb, _extends({
        ...
        children: [(0, import_jsx_runtime27.jsx)(Input3, _extends({
          ...
          value: values[index2]
        }, inputProps)), ValueLabel ? (0, import_jsx_runtime27.jsx)(ValueLabel, _extends({}, !isHostComponent_default(ValueLabel) && {
          valueLabelFormat,
          index: index2,
          disabled
        }, valueLabelProps, {
...
var TabPanel = React43.forwardRef(function TabPanel2(props, forwardedRef) {
  var _slots$root;
  const {
    children,
    slotProps = {},
...
var Tab = React45.forwardRef(function Tab2(props, forwardedRef) {
  var _slots$root;
  const {
    children,
    disabled = false,
...
function CssAnimation(props) {
  ...
  const hasExited = React46.useRef(true);
  React46.useEffect(() => {
    if (requestedEnter && hasExited.current) {
      hasExited.current = false;
    }
  }, [requestedEnter]);
  const handleAnimationEnd = React46.useCallback((event) => {
...
var CssTransition = React47.forwardRef(function CssTransition2(props, forwardedRef) {
  const {
    children,
    className,
  ...
  const [isEntering, setIsEntering] = React47.useState(false);
  React47.useEffect(() => {
    if (requestedEnter) {
  ...
  const handleTransitionEnd = React47.useCallback((event) => {
...
@mui/base/index.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-D65WQBSY.js.map
