...
let BrandsService = class BrandsService {
    brandRepositry;
    appService;
    constructor(brandRepositry, appService) {
    ...
    async findAll() {
        ...
        if (!allBrands || allBrands.length === 0) {
            throw new common_1.NotFoundException("There is no brands data Available");
        }
        return allBrands;
    }
    async findOne(id) {
    ...
    async update(id, updateBrandDto) {
        ...
        if (!updatedata) {
            throw new common_1.NotFoundException('Brand Not found for update = failed');
        }
        return updatedata;
    }
    async remove(id) {
